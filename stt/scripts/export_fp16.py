#!/usr/bin/env python3
"""Export fine-tuned Whisper to FP16 folder for Jetson deploy."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--dst",
        type=Path,
        default=STT_ROOT / "models" / "export" / "en-hi-base-v1-fp16",
    )
    ...
    processor = WhisperProcessor.from_pretrained(str(args.src))
    model = WhisperForConditionalGeneration.from_pretrained(str(args.src))
    model = model.half()
    args.dst.mkdir(parents=True, exist_ok=True)
    model.save_pretrained(str(args.dst))
    processor.save_pretrained(str(args.dst))
    ...
    print("Note: on Jetson, load with torch + CUDA; half() weights reduce disk/RAM.")
    return 0
